...
| `record_evidence` | Record one result, or many at once, and fire the resulting transitions. **Batch-native**: pass `results`, a **list** of `{node_id, success, depth, claim_id, …}`, to report every experiment from one turn in one call — they are applied **in order**, so a refutation's cascade lands before the next result is read. Pass the single-result fields directly for the `k=1` case. `attestation_id` may reference immutable process metadata minted beforehand by a trusted in-process runner; callers can cite the id but cannot supply runner fields. Known attestations derive duration, default source, and commit context; unknown ids safely degrade to self-reported evidence. Pass `evidence_kind:"infra"` for infrastructure errors and omit `success`: no logical measurement occurred, and infra evidence never invalidates. `count_next_targets` is a top-up, not an addition. A single result returns `{node, next_targets}`; a batch returns `{recorded, failed, next_targets}`. |
...
| `get_goal_status` | Goal nodes + progress counts, `frontier_size`, `total_nodes`, `status_breakdown`, `stale_beliefs`, `unwired_confirmations`, and `ceiling_settled`. With `goal_id`, unwired confirmations are VERIFIED nodes outside that objective's support. |
...
| `get_evidence_history` | Evidence trail for a node (newest-first), including artifacts, `attestation_id`, `verified_by`, and any attestation/context mismatch. |
...
An attestation is process metadata minted through the direct Python API by the runner that
observed execution. It records command arguments, revision, exit code, duration, and output
digests. Tool callers may pass only `attestation_id`; no MCP/tool schema can mint or fill the
runner-owned fields. Attested and self-reported evidence with the same numbers have identical
posterior semantics, but evidence history distinguishes their provenance. A context supplied
by the caller that disagrees with the attestation is retained and flagged rather than hidden.
...
2. **`success ∈ [0,1]` for logical evidence only.** Deterministic host checks should use exactly `0.0` or `1.0`; represent partial progress as a narrower claim. Infrastructure reports use `evidence_kind="infra"` and omit `success`. Pass raw metrics in `metrics`, rigour in `depth`, and runner provenance by `attestation_id` when available.
