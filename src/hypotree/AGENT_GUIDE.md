...
2. **`success ∈ [0,1]`** — normalized reward. Deterministic host checks should use exactly `0.0` or `1.0`; represent partial progress as a narrower claim. Pass raw metrics in `metrics`, rigour in `depth`, and runner provenance by `attestation_id` when available.
